...
import scripts.api.events.banking.BankCache;
import scripts.api.events.banking.BankEventV2;
import scripts.api.events.inventory.InventoryEvent;
    ...
    public void clickConfirm() {
        ...
        if (Interfaces.isInterfaceSubstantiated(confirmButton)) {
            if (confirmButton.click()) {
                GrandExchange.WINDOW_STATE state = GrandExchange07.getWindowState();
                Sleep.until(() -> state != null && state.equals(GrandExchange.WINDOW_STATE.SELECTION_WINDOW));
            }
        }
    }
